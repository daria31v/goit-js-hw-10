...
function onSearch(evt) {
    const inputUser = evt.target.value.trim()
...
function createCountryCard(arr) {
    const markupCard = arr.map(({ name, capital, population, languages, flags}) => 
      ...
      <p class="info-country">Languages: ${languages}</p>
      <img src="${flags.svg}" alt="flag coutry" />
      </div>`).join('');
...
function createCountryList(arr) {
    const markupList = arr.map(({ flags, name }) =>
      ...
      <img src="${flags.svg}" alt="flag coutry" width="50" height="30" />
